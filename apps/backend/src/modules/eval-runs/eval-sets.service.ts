...
import type { Tx } from "../../platform/persistence/persistence.module";
...
export class EvalSetsService {
  ...
  /**
   * 批量建 draft 用例，**全程在调用方的事务里**（`GapPromoteService.promote` 的落库半边）。
   *
   ...
   */
  async createCasesBatchTx(
    tx: Tx,
    setId: string,
    entries: CreateEvalCaseRequest[],
  ): Promise<EvalCase[]> {
    if (!(await this.repo.findSetByIdTx(tx, setId))) throw new NotFoundException("评测集不存在");
    const created = await this.repo.insertCasesWithVersionsTx(
      tx,
      entries.map((req) => ({
        setId,
        sourceTraceId: req.sourceTraceId,
