...
it("回复未引用检索（正文无 [n]）时不显示可信度/引用完整度", async () => {
  vi.spyOn(sse, "openChatStream").mockImplementation(() =>
    stream([
      { type: "citation", citation: { n: 1, doc: "d", kb: "售后库", section: "s", score: 0.3, text: "x" } },
      { type: "token", delta: "抱歉，这个问题我无法从知识库中找到依据回答。" }, // 无行内 [n]
      { type: "done", traceId: "t4", convId: "c4", confidence: 0.4, coverage: "partial", isFallback: false, fallbackReasons: [] },
    ]),
  );
  renderAt();

  fireEvent.change(await screen.findByPlaceholderText(/输入您的问题/), { target: { value: "闲聊一下" } });
  fireEvent.click(await screen.findByRole("button", { name: /发\s*送/ }));

  await waitFor(() => expect(screen.getByText(/抱歉，这个问题我无法/)).toBeInTheDocument());
  expect(screen.queryByText(/可信度/)).not.toBeInTheDocument();
  expect(screen.queryByText(/引用完整/)).not.toBeInTheDocument();
});
