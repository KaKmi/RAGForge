...
7. **回验没有「只重跑、不重新入库」的路径**（B2b 新增）。`DocumentChangeNotifier` 的终态事件只广播一次；若那一次判分因判官服务瞬时故障没得出分数，簇走 `verifyInconclusive` 回 `pending`（不打复发标），用户要重新走一遍向导——**新文档、新入库**——才能再触发一次真实回验。
...
8. **补库向导曾因 state 跨簇存活而可把「簇 A 的问答提交进簇 B」**（B2b 第三轮 peer review 抓出，已修）。
