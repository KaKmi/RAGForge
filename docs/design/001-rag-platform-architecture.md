...
related: ["design/007", "design/009", "design/010", "design/011", "design/012"]
...
2026-07-10：真实 PDF/文章来源需求触发文档处理边界修订：引入版本化 Processing Profile、Canonical Document、版面解析/OCR 与不可变处理 Run，详见 010。该修订取代本文中“解析/清洗固定、仅双模板”的旧约束，但不开放任意数据工程编排。
2026-07-10：Prompt 可编辑与固定 Agent 节点之间增加版本化 NodeContract、三层 Prompt 组装、Agent 激活门禁和运行时结构校验；PromptVersion 固定 ContractVersion，非法模型输出不得进入编排，详见 011。
2026-07-11：原型将“Agent 管理”重构为“应用管理”：应用配置版本全部平等且不可变，单一 `production_config_version_id` 指向当前服务版本；新建 v1 默认未上线，上线前异步执行真实 NodeRuntime 预演，停用为独立总开关。该目标设计替代 `current_version_id + draft/published/archived + Eval stub` 发布状态机，详见 009。现有代码尚未迁移，因此本文保持 `draft`。

## Summary

CodeCrushBot 是一个**通用 RAG 平台**（课程问答只是示例 mock 数据）。它把一次问答拆成可配置、可追踪、可优化的流水线：模型接入 → 知识库/结构化入库 → 应用配置 → 问答/检索 → **Trace 追踪**。文档入库以版本化 Processing Profile 组合解析、清洗和分块，统一产出可溯源 Markdown；核心技术抉择仍是后端 NestJS 自研薄编排、数据面 Postgres+pgvector 单库，以及 **OpenTelemetry SDK → Collector → ClickHouse** 的标准 OTLP 可观测链路。本地 docker-compose 优先，阿里云托管服务（Tier B）设计就绪、择期落地。
...
- 知识库 / 文档 / 切片：**Profile 驱动的四阶段管线**（解析→清洗→分块→向量化），知识库默认方案 + 文档覆盖、Canonical Document、版面解析/OCR、处理 Run 快照、切片查看/搜索/批量删除、文档元数据与版本化蓝绿重建（详见 007、010）
- 应用配置（绑定 KB、四节点模型、4 个不可变 PromptVersion、对应 NodeContractVersion、检索参数和兜底策略）；应用版本平等且不可变，production 指针切换前执行静态检查与异步真实 NodeRuntime 预演（009、011）
- 问答/检索：RAG 编排 + 检索测试台；rewrite/intent 等程序消费节点只接收通过 NodeContract 校验的 typed output（011）
...
- ~~解析/清洗固定默认实现、分块只开放「通用/问答」两模板~~（2026-07-10 被 010 的受控 Profile 设计取代；当前代码已有 `general / qa / custom` 三种分块器）
- **任意数据处理工程**：用户脚本、上传插件、自由拖拽 Pipeline、客户端任意组合 Parser/Cleaner/Chunker 仍不做；只开放服务端注册、版本化、经过测试的 Profile
- HA / 水平扩展 / 多区域
...
6. **模型原始输出不进入编排**：结构化节点必须经固定 ContractVersion 的输出 Schema 和动态值域校验；失败最多修复一次，仍失败执行确定性 Fallback（011）。
7. **生产 Prompt/Contract 不漂移**：ApplicationConfigVersion 固定 PromptVersion，PromptVersion 固定 ContractVersion；新版本不会静默改变线上应用（009、011）。
8. **`production_config_version_id` 是应用线上配置唯一事实源**：Prompt 标签没有应用发布语义；公开问答只解析 production 指针，停用开关优先于指针（009、012）。
...
`auth`（JWT）· `models`（模型注册 + `ModelProviderPort`，首个适配器 OpenAI 兼容，覆盖 DeepSeek/Qwen/GPT/Claude/vLLM）· `knowledge-bases` · `documents`（上传/生命周期/Profile 覆盖）· `ingestion`（Profile 注册与 Run 编排：解析→清洗→Canonical Document→分块→向量化，见 007、010）· `chunks` · `retrieval`（`RetrieverPort`：向量+关键词多路召回→融合→重排，**chat 与检索测试台共用**）· `applications`（不可变配置版本、production 指针、异步 ReleaseCheck 与运行时解析，见 009）· `prompts`（不可变版本、标签、编译和试运行，见 012）· `node-runtime`（版本化 NodeContract、Prompt 编译/组装、结构化输出、预览/预演/降级，见 011）· `chat`（RAG 编排 + SSE 流式，只消费 typed node output，产出完整 OTLP trace）· `traces`（只读 ClickHouse）· `conversations`。
...
- `knowledge_bases(id, name unique, desc, default_profile_id, default_profile_version, embedding_model_id, status[ready/building/failed], active_version, building_version)` — Embedding 创建后锁定；Profile 显式应用到旧文档时触发版本化蓝绿重建（007、010）；迁移期保留 `chunk_template`
- `documents(id, kb_id, name, type, size, status[pending/queued/processing/failed/ready], blob_key, profile_override_id/version, metadata jsonb, chunk_version, lifecycle jsonb, error, uploaded_at, updated_at)`；`document_processing_runs` 保存不可变 Profile Snapshot、解析引擎、Canonical Document 引用、统计与终态（010）
- `chunks(id, doc_id, kb_id, version, processing_run_id, seq, text, token_count, section, content_type, page_start, page_end, asset_key, embedding vector(1024), tsv tsvector[M5 回填])` — 向量(pgvector HNSW) + 关键词(FTS) 同表；~~enabled~~ 已改删除制（007）；检索恒过滤 `version = kb.active_version`
- `applications(id, slug, name, description, enabled, production_config_version_id, deleted_at, created_by, updated_by, created_at, updated_at)`；production 单指针是服务版本唯一事实源（009）
- `application_config_versions(id, application_id, version, config_schema_version, prompt_*_version_id×4, *_model_id×4, node_params, retrieval_params, fallback_params, note, created_by, created_at)`；业务字段不可变、无发布状态（009）
- `application_config_version_kbs(version_id, kb_id)`；版本级知识库快照
- `application_release_checks(id, application_id, config_version_id, config_fingerprint, status, issues, sample_summary, expires_at, created_by, created_at)`；异步真实预演的短期 artifact（009）
- `prompts(id, name, node[rewrite/intent/reply/fallback], updated_by, created_at, updated_at)`
- `prompt_versions(id, prompt_id, version, body, variables jsonb, contract_version, compile_status, compile_errors jsonb, note, author, created_at)` + `prompt_version_tags(prompt_id, prompt_version_id, name, created_by, created_at)`；Prompt 标签不改变应用，应用固定引用具体版本（012）
- `conversations(id, application_id, application_name_snapshot, user_id, title)` · `messages(id, conv_id, role, content, trace_id, confidence, citations jsonb)` — **`trace_id` 是可追踪关键外键**
...
- `NodeRuntimeService.execute(node, contractVersion, promptBody, modelId, input, context) -> TypedNodeOutput`：统一编译、三层组装、Structured Output、Schema/动态值域校验、一次修复和 Fallback；Prompt 试运行、应用 ReleaseCheck 与 chat 共用（011）
- `ApplicationConfigResolver.resolvePublic(applicationIdOrSlug) -> ResolvedApplicationConfig`：只解析 production 指针，按 deleted→disabled→not-published→available 的顺序返回；管理员测试显式传 configVersionId（009）
...
- **LLM 节点输出非法**：按固定 ContractVersion 校验失败 → 修复一次 → 仍失败执行节点 Fallback；原始 JSON/文本绝不流入检索或路由。ContractVersion 缺失则实例 readiness 失败，不以“最新版本”替代（011）。
...
- 无 RBAC → 加：多团队
- Processing Profile 超过 10 个或需运营无部署配置 → 从代码注册表升级为受约束的声明式 Profile；仍不开放任意脚本（010）
- 活跃 NodeContractVersion >3 或结构修复/Fallback 连续 100 次 >3% → 启动契约迁移与模型能力治理（011）
...
- 011：Prompt 三层组装、版本化 NodeContract、Prompt 试运行/应用预演与运行时结构化输出校验
- 012：Prompt 版本平权、标签模型、试运行和应用生产使用关系派生查询
