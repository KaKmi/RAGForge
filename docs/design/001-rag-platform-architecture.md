...
related: ["design/007", "design/010", "design/011"]
...
- Agent 配置（绑定 KB、三类模型、4 个不可变 PromptVersion、对应 NodeContractVersion、检索参数、激活门禁、兜底转人工）
...
7. **生产 Prompt/Contract 不漂移**：AgentConfigVersion 固定 PromptVersion，PromptVersion 固定 ContractVersion；新版本不会静默改变线上 Agent（011）。
...
- `prompt_versions(id, prompt_id, version, body, variables jsonb, contract_version, compile_status, compile_errors jsonb, note, author, status[draft/prod/archived])` — body/node/contractVersion 不可变；Agent 配置固定引用具体版本。Prompt 状态/标签的产品重构独立演进，不改变 011 契约固定语义
...
- `NodeRuntimeService.execute(node, contractVersion, promptBody, modelId, input, context) -> TypedNodeOutput`：统一编译、三层组装、Structured Output、Schema/动态值域校验、一次修复和 Fallback；预览、Agent 激活与 chat 共用（011）
...
- 011：Prompt 三层组装、版本化 NodeContract、Agent 激活门禁与运行时结构化输出校验
