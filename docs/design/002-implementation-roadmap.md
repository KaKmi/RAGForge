...
- **E-W2b 技术债收口**——018 §12 缺口 **13/14/15/18**（用户此前指示「先做功能，技术债后修」，现功能已完），外加 refactor 波推迟的 `computeGoldCoverage` 投影过取（屏3 每 3s 轮询拉全量快照字段只用一个布尔）。
- **E-W3 / E-W4**——`018` §1 仍 OUT 的三项：**问题池**、**加入评测集**（在线 → 离线回流）、**gold_stale 检测器**。这是把「在线发现问题 → 沉淀成 gold → 回归验证」飞轮真正闭上的一波。
- **M4.1b**——Docling / OCR / 表格图片结构块 / 资源归档（Rollout 5–6），见 `010` Status。
- **M10 收尾**——4 个前端测试待收尾（见 PR #28 描述）+ cost 真算（M9/M10 两处均延后，需先定 pricing 来源）。

已收口：**M9 首期**（#21/#22/#23/#24/#25 全合；cost 真算按用户决策移出首期）· **M10 运行看板**（#28 合）· **M11 的 E-W1**（#31 合）**与 E-W2a**（#32 合）· **E-W2b-0 worker 拆进程**（#35）· **E-W1 屏1 口径修复**（[#36](https://github.com/KaKmi/RAGForge/pull/36)）· **Judge scoring integrity v2**（020 / `0021_judge_scoring_v2`，[#38](https://github.com/KaKmi/RAGForge/pull/38)）· **E-W2b 功能波**（[#39](https://github.com/KaKmi/RAGForge/pull/39)）。
