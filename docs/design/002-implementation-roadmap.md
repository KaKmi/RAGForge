...
**当前下一步**：**未定，待用户拍板**。E-W2b 功能波已于 2026-07-18 交付并合入 main（[PR #39](https://github.com/KaKmi/RAGForge/pull/39)），评测飞轮的离线主干（gold 题库 → run 引擎 → 8 指标记分卡 → 屏3 报告 → 屏4 版本对比 → 单条重放）已闭环。候选下一波（按依赖无先后，需用户选）：
...
| **B1 上线门禁 + 单条回流三件** | **F5** 上线门禁（**软提示**：挂 M7b 既有 ReleaseCheck，门禁结论以 `severity:"warning"` 级 issue 进既有 `issues[]`，`status` 仍 `passed`，**后端发布路径零拒绝**；应用级开关 `eval_gate_enabled` 默认 `false` ⇒ 存量应用发布行为逐字节不变，开关只控前端是否 disable「去上线」按钮）+ **F2** Trace 详情「加入评测集」（含「已在评测集 · 查看」两态）+ **F3**「立即评测」端点与评分中/重试态（独立 `MANUAL_SCORE_QUEUE`，api 角色）+ **F4** `gold_stale` 检测器与「确认仍有效」。收口 018 §1 OUT 的三项中的两项与缺口 4 | **本波已实现，待 PR**（分支 `ship/eval-reflux-gate-release-gate-soft-warning-plus-add-to-eval-`）。迁移 **0024**（`eval_gate_enabled`）、**0025**（`eval_manual_score_jobs`）。**核心不变量守住**：离线/重放分数只落 PG 绝不发 `rag.eval` span、NULL 不退化 0、线上 chat 路径逐字节不变、`offline-v2`/`online-v2` 均不升号（`rag.eval.trigger` 是纯附加属性，默认 `worker`，读路径与 `eval_1m` MV 零感知）、「立即评测」**不推进水位线、不写 `eval_candidate_ledger`**。**peer review 抓到并已修的两处要害**：① 人工评测的重复抑制是空的（pg-boss `singletonKey` 在 `standard` 策略下惰性 + 进程内限频挡不住多副本 ⇒ 双击可付两次裁判钱），改 `ON CONFLICT … DO UPDATE … WHERE` 原子占位；② `getLatestFailure` 无版本/时间下界 ⇒ 失败 span 永久压过作业行，**「重试」这条路根本走不通**，改按时间取新。③ gold 过期检测最初漏掉 `KbRebuildService`（整库重建绕过 documents 域，量最大的一类），广播点提到平台层 `DocumentChangeNotifier`。**⚠️ 本波未跑真库/真 infra**：本机 Docker daemon 不可达（`com.docker.service` 停止，未擅自启动 Windows 服务）⇒ `test:db` / `test:infra` 一次未跑，`check-test-baseline.mjs` 的 infra 基线 `7/82` 是**推算值**（73 + 新 spec 9 条），**下一次 infra 全绿后必须以实测覆盖**。fast 层：backend 1091 绿 / frontend 23 文件绿 / contracts 20 绿 / lint 0 / build 5/5。**剩余 B2**：屏5 问题池、从坏样本生成、首页质量摘要行、gold 按章节锚点自动重定位 |
