...
| **M11** | 评测集 / 管理 / 报告 | 🔄 | 原列「里程碑 2 不做」，实际已按**评测飞轮 E-W 波次**推进：**E-W1 在线质量**（PR #31）+ **E-W2a 离线 run/评测集**（[PR #32](https://github.com/KaKmi/RAGForge/pull/32)，含运行时 QA 修复）+ **E-W2b-0 worker 拆进程**（[PR #35](https://github.com/KaKmi/RAGForge/pull/35)，019/018 缺口 19）+ **游标语义收口（账本）**（迁移 `0020_secret_nightmare`）+ **Judge scoring integrity v2**（020，迁移 `0021_judge_scoring_v2`；**2026-07-18 第二轮修复+真实验证已交付**——首版 `.max(100)` 假设被真实调用证伪，改为归一化+修复式重试后 32 条真实 trace 裁判失败数 0/32）——均已交付，**[PR #38](https://github.com/KaKmi/RAGForge/pull/38)**；**E-W2b**（重放/对比屏4/检索层指标）待做；~~E-W2c 思考 token 治理~~ **已由换模型解决、波次取消**（2026-07-17 实测）——见附录 A「评测飞轮（E-W）分波交付进度」 | 017 / 018 / 019 / 020 |
| **M12** | RBAC 权限 | ⬜ | 里程碑 2（首期不做） | — |

**当前下一步**：**E-W2b 功能波**（重放 / 版本对比屏4 / 检索层 gold-docs 指标）——范围见附录 A 的 E-W2b 行与 `018` §1「OUT（W2b 及以后）」。**注意**：2026-07-17 起用户连插三个小波（`E-W2b-0` worker 拆进程 [PR #35](https://github.com/KaKmi/RAGForge/pull/35)、`E-W1 屏1 口径修复` [PR #36](https://github.com/KaKmi/RAGForge/pull/36)、`游标语义收口` 账本，均已交付，见附录 A 对应行），故 E-W2b 功能波**尚未开工**，仍从 main 开新分支。另外 **`Judge scoring integrity`**（020）刚完成第二轮修复+真实验证（2026-07-18，见附录变更记录），当前尚**未提交 git**（仅本地 working tree），下一步需先确认是否提交/开 PR，再决定是否顺带开 E-W2b。
