...
**当前下一步**：**E-W4 已完结**（B2a + B2b 均已交付）。下一波待用户指定——候选见附录 A「B2b」行末的「B2b 之后仍欠什么」。

> **B2a 已交付**（2026-07-19，[PR #46](https://github.com/KaKmi/RAGForge/pull/46)）：
> 屏5 问题池 + 收集器 worker + 从坏样本生成 + 首页质量摘要行（[PR #46](https://github.com/KaKmi/RAGForge/pull/46)）。**但 B2a ≠ E-W4 完结**，见附录 A「B2a」行的「B2b 仍欠什么」。

> ⚠️ 开工前先读 [`018` §12.x](018-offline-eval-runs.md)「真实数据实测暴露的待修项」（缺口 23–26，2026-07-19）。用户明确指示：**先做完 E-W4 闭环，这些 bug 慢慢修**，故 B2 不必捎带修它们——但缺口 23（多轮指代未被改写消解，导致精确率 0 → 走兜底）是**真实产品缺陷**，问题池的信号质量会直接受它影响，设计时心里有数。
>
> ~~🔺 **缺口 23 已提级为 E-W4 之后的第一优先**~~ —— **同日撤回**。提级的理由是「它卡死了 B2b 闭环」，而进一步查证证明卡住闭环的是**另一个 bug**：手动入池时把 rewrite 的结果扔了（`manual_trace` 明明走过 rewrite 节点，结果就在 `rag.rewrite.query` 里）。`ctxprec` 恒 0 是那个 bug 的**症状**，不是原因。修复后同日实测：补库回验 **88 分 ⇒ 已回验✓**，沉淀的 gold 用例跑离线评测 **Context Precision/Recall/命中率@5 全 100、判定通过**——**E-W4 两条飞轮臂首次完整闭环**。缺口 23 仍是真实缺陷，维持 `018` 原有的「慢慢修」定级。全过程（含两次误诊）见 [`021` §11.7](021-problem-pool-gap-domain.md)。
> 另：[PR #44](https://github.com/KaKmi/RAGForge/pull/44) 起 ReleaseCheck 第二段冒烟采样默认关闭（`RELEASE_CHECK_SAMPLING_ENABLED=false`），上线校验现在是毫秒级、只跑静态门禁。
...
- ~~**E-W3 / E-W4**——`018` §1 仍 OUT 的三项：**问题池**、**加入评测集**（在线 → 离线回流）、**gold_stale 检测器**~~
  **「加入评测集」与「gold_stale 检测器」已由 B1 波交付**（含上线门禁软提示、「立即评测」端点与评分中/重试态）——见附录 A「B1」行。
  ~~**仍 OUT 的只剩「问题池」**，归 B2~~ **问题池核心已由 B2a 交付**（2026-07-19）；剩余的 [补知识库] 向导与自动回验归 **B2b**，见附录 A 的「B2a」/「B2b（待做）」两行。
...
| **B2a 问题池（评测飞轮臂）** | E-W4 的**核心闭环**：`gaps` 新域（021，依赖顶点）+ 迁移 **0026/0027** 三表 + 收集器 worker（游标/租约/增量聚类/分诊/幂等）+ 屏5 问题池页面 1:1 还原 + 两处入池入口（Trace 详情 / 屏3 行尾）+ 「从坏样本生成」批量沉淀 gold + 首页答案质量摘要行 | **已交付**（2026-07-19，[PR #46](https://github.com/KaKmi/RAGForge/pull/46)，待合并）。**核心不变量守住**：`gaps` 是依赖顶点（eslint Boundary ⑤ 机械保证，无人 import 它）；离线分数仍只落 PG 不发 `rag.eval` span（`test/gap-pool-isolation.spec.ts` 在真 PG+真 CH 上钉住「跑一轮收集，屏1 的 getOverview/getLowSamples 逐字段不变」）；收集器**绝不写** `eval_watermarks`/`eval_candidate_ledger`，用自有 `gap_watermarks`；用例恒 `draft`（无人审不入集）；NULL 不退化 0（`avgQuality` 无分显「未评」）。**每 story 独立 peer review**，抓到并已修的要害：① 构造参数写成接口交叉类型 ⇒ `emitDecoratorMetadata` 吐 `Object` ⇒ 接线即启动崩（tsc 不报、单测/e2e 都绕过 DI，已加 `design:paramtypes` 守护网）；② drizzle 在 select 投影里把列对象渲染成**不带表名**的 `"id"`，放进相关子查询会解析成内层表的列 ⇒ 三个聚合**静默全归零不报错**；③ 重投时质心漂移会遗留 `freq=0` 空簇（先探后建 + 哨兵回滚）；④ 游标经 PG 时间戳往返丢纳秒 ⇒ 末行每轮重取、**永久卡死**（迁移 0027 改 varchar）；⑤ `followUpRatio` 分母一度被"对齐"成含手动样本 ⇒ 补证据反而让 §6.4 的强制改判失效、根因翻回 `missing`。测试：`test:db` **13 suites/117**、`test:infra` **9 suites/106**（均实测、跳过 0）、后端 1194 绿、前端 312+ 绿、contracts 254 绿、lint 0、tsc 0 | 
| **B2b** | 问题池剩余：**[补知识库] 三步向导**（LLM 草拟 → 人审编辑 → 入库走现有管线，红线「无人审不入库」）+ **入库后自动回验**（重放代表问题 + 重新评分，≥80 转「已回验✓」并显示 41→89）+ **缺口状态机四态**（`drafting/reviewing/filled/verified`，需 `ALTER` `gap_clusters_status_check`）+ 「复发」角标 + 屏3 行尾「标记忽略」（**需要先给 `eval_run_results` 加可落该标记的字段**——B2a 明令不改 eval-runs schema，故未做）+ 入集重复检测的完整版（B2a 降级为「本批内部去重」，未与目标集既有用例做 >0.95 相似度比对） | **已交付**（2026-07-21，[PR #47](https://github.com/KaKmi/RAGForge/pull/47)，CI 全绿待合并）。决策 I–N 见 [`021` §9b](021-problem-pool-gap-domain.md)；迁移 **0028**（CHECK 扩七态 + `gap_clusters` 9 新列 + `eval_run_results.ignored_at`）/ **0029**（`terminal_at`）。**红线守住**：LLM 草拟内容必须过 `reviewing` 人审节点才能入库——服务端在 upload **之前**硬拦状态，前端另有独立一道（勾「已核对」才可提交）。**每 story 独立 peer review**（完整对抗档），抓到并已修的要害：① 我自己在 `submitFill` 里把 upload 写在了状态检查**之前**，等于绕过本波唯一的红线（被自己的测试抓住）；② 回验的 `failed` 分支**生产不可达**——`notifyContentReplaced` 只在 `ready` 广播，补库文档解析失败会让簇永久卡在 `filled`（该态只剩「忽略」可走），故在平台层新开「文档终态」通道；③ 复发窗口按滚动 7 天算 ⇒ 刚被忽略的热门簇会被它**忽略之前**的历史立刻重开，「忽略」变成无效操作（迁移 0029 补 `terminal_at` 锚点）；④ 前端向导单实例常驻、state 跨簇存活 ⇒ 「簇 A 的问答被提交进簇 B」两条路径（load 失败残留 / 陈旧响应覆盖），复审用探针实测复现；⑤ 三轮复审共抓出**两条空测**（其中一条我自己做过变异检查却漏了，另一条的守卫写成比闭包 `clusterId`、恒成立等于没写——正是被那条空测「验证」通过的）。测试：`test:db` **14 suites/166**、后端 **1277**、前端 **350**、lint 0、tsc 0。**B2b 之后仍欠什么**：全量问题聚类（M10 看板「热门问题」，见 `021` §11.6 的复用提示）；「仅重跑回验、不重新入库」的路径（当前判官故障需重走整个向导，见 `021` §11 新增取舍） |
