...
> **B2a 已交付**（2026-07-19，分支 `feat/eval-w4-problem-pool`，**尚未推送/开 PR —— 合并后请把 PR 链接补进附录 A 的「B2a」行**）：
> 屏5 问题池 + 收集器 worker + 从坏样本生成 + 首页质量摘要行。**但 B2a ≠ E-W4 完结**，见附录 A「B2a」行的「B2b 仍欠什么」。
