...
**当前下一步**：**B2b 波**——问题池的「补知识库」向导 + 自动回验 + 缺口状态机四态（`drafting/reviewing/filled/verified`）。
...
| **B2b（待做）** | 问题池剩余：**[补知识库] 三步向导**（LLM 草拟 → 人审编辑 → 入库走现有管线，红线「无人审不入库」）+ **入库后自动回验**（重放代表问题 + 重新评分，≥80 转「已回验✓」并显示 41→89）+ **缺口状态机四态**（`drafting/reviewing/filled/verified`，需 `ALTER` `gap_clusters_status_check`）+ 「复发」角标 + 屏3 行尾「标记忽略」（**需要先给 `eval_run_results` 加可落该标记的字段**——B2a 明令不改 eval-runs schema，故未做）+ 入集重复检测的完整版（B2a 降级为「本批内部去重」，未与目标集既有用例做 >0.95 相似度比对） | ⬜ 待做 |
