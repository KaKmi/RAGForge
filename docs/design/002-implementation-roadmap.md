...
| **M7b** | 应用发布闭环 | ✅ | | 009 |
| **M8** | 问答 / RAG 编排 | 🔄 | **T1 已交付**（PR #16）；**T2 已交付**（PR #18）；**T3 已交付**（PR #19，2026-07-13，真实 infra+LLM QA 验证 5 项写侧数据 ClickHouse 落库）；**T4 待做**（见附录 A） | 013 / 014 |
| **M9** | Trace 追踪（完整版） | ⬜ | 下一大里程碑 | 待设计 |
| **M10** | 运行看板 | ⬜ | 里程碑 2（首期不做） | — |
| **M11** | 评测集 / 管理 / 报告 | ⬜ | 里程碑 2（首期不做） | — |
| **M12** | RBAC 权限 | ⬜ | 里程碑 2（首期不做） | — |

**当前下一步**：M8 T4（C 端问答页 `/chat` 真实接 `/api/chat`：三栏、行内角标 ⇄ 右栏原文、可信度/引用完整度、兜底卡、复制/反馈/转人工——先 1:1 读原型再还原）——见附录 A「M8 分波交付进度」。
...
> 提示（给后续会话）：M8 已交付 **T1/T2/T3**（编排内核 + SSE 流式 + trace 写侧富化），**T4（C 端问答页）待做**；判断「下一步」应从 T4 起，勿据「M8 代码已合并」推断 M8 完结。
