...
| **M7b** | 应用发布闭环 | ✅ | | 009 |
| **M8** | 问答 / RAG 编排 | 🔄 | **T1 已交付**（PR #16）；**T2 已交付**（PR #18，2026-07-13，真实 infra+LLM QA 验证逐 token/span 树/`message.trace_id`）；**T3/T4 待做**（见附录 A） | 013 / 014 |
| **M9** | Trace 追踪（完整版） | ⬜ | 下一大里程碑 | 待设计 |
| **M10** | 运行看板 | ⬜ | 里程碑 2（首期不做） | — |
| **M11** | 评测集 / 管理 / 报告 | ⬜ | 里程碑 2（首期不做） | — |
| **M12** | RBAC 权限 | ⬜ | 里程碑 2（首期不做） | — |

**当前下一步**：M8 T3（trace 写侧富化：检索 span 三拆 / `gen_ai.usage.*` / 命中分表 / 质量信号 / PII 脱敏）——见附录 A「M8 分波交付进度」。
