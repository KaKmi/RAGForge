...
20. **屏1（E-W1）的三处口径问题 —— 用户 2026-07-17 截图提出，实测证实**（**非本波引入；E-W1 地盘，待独立小波**）——
...
21. **`processed_failed` 也永久跳过 —— 缺口 20(b) 的遗漏项**（2026-07-17 新发现）——
...
22. **`FaithfulnessEvaluator` 让屏1 的事实一致性成为反向指标 —— 全量实测，P1**（2026-07-17，**需产品裁决**）——
    ...
    **为什么本波不改**：`faithfulness.evaluator.ts` 是 **E-W1 冻结基线**；schema 即**解析契约**，
    改它触发 `017` 的 `judgeVersion` 升版要求 ⇒ 历史在线分数断代（同缺口 18 的处境）。
    **属产品决策不是实现细节。**
...
19. **评测 worker 与 API 同进程 —— 该拆独立部署物**（**用户 2026-07-17 提出，待独立波次**）——
...
18. **忠实度判分依据缺原型 §7 的逐条「支持/不支持」**（QA 发现，**需产品决策，本波不改**）——
