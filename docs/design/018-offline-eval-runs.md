...
22. **`FaithfulnessEvaluator` 空 claims → 100 分：从假设变实测**（2026-07-17）——
    缺口 10 曾把它记为「需 W2b 决策」的**理论**路径。2026-07-17 的受控实测**坐实了它**：
    `c4669188`（`status=fallback`、`no_citations=1`，即系统没答出来、零引用）经真实裁判
    （deepseek-v4-flash / online-v1）评出 **`faithfulness=100` / `answer_relevancy=30` / `context_precision=0`**。
    **一个答不出来的兜底回答，在记分卡上事实一致性满分。** 且这是**在线**路径（不只是缺口 10 担心的离线），
    直接进屏1 的 avg。缺口 10 的决策（保留「空断言=满分」语义 vs 降为「无可判定 → 未评」）由此有了实测样本。
