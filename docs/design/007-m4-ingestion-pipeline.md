...
M4 把 M2 的知识库三屏 mock 换成真实现：`knowledge_bases`/`documents`/`chunks` 三张表落 Postgres+pgvector，`platform/storage`（BlobStore 端口 + LocalFs 适配器）与 `platform/queue`（pg-boss 同进程 worker）两块平台件就位，`ingestion` 模块实现**四阶段端口化可插拔管线**（解析→清洗→分块→向量化，整条 pipeline 本身也是可替换端口）。核心新机制是**切片版本化蓝绿重建**：`chunks.version` + `knowledge_bases.active_version/building_version`，支撑「改分块模板触发全库重建、重建期间检索用旧版本、完成后原子切换」。产品权威来源为原型目录《知识库模块-产品设计》（2026-07-08）与 `CodeCrushBot.dc.html` 最新版。
...
**In-scope**
- KB CRUD：新建（名称查重 / 描述 / 分块模板二选一 / Embedding 从已启用模型任选）、编辑（模板可改并触发全库重建，Embedding 创建后锁定）、列表卡片精简态（常态无状态标签，重建中蓝色呼吸点+百分比）、详情页流水线配置摘要行。
- 文档：单文件/文件夹批量上传（PDF/Word/Markdown/TXT，文件夹模式带相对路径）、「上传后立即解析」开关（关→待解析+手动开始）、三态宏观生命周期（上传→解析入库→就绪）+ 失败原因 + 重试、文档级键值元数据（jsonb）、删除。
- 管线：解析→清洗→分块→向量化四阶段，每阶段端口化；整条 pipeline 亦为可替换端口。分块模板：通用（标题结构）/ 问答（一问一答一切片）。
...
- 「知识库→数据源→文档」三层架构；按内容类型细分模板矩阵；清洗规则自定义 UI；父子分块/Small-to-big；单文件覆盖库默认模板；分块样例预览闭环；编辑时更换 Embedding 模型。
- OCR / 扫描件识别（解析失败给明确原因）。
...
| 扫描件/空文本 PDF | 解析阶段 fail，原因明示，不自动重试 |
...
| 解析器 | pdf-parse + mammoth 纯文本抽取 | OCR/版面识别引擎 | 扫描件明确失败 |
...
- 001（系统架构，本文对其数据模型/管线描述做了 M4 修订）· 002（路线图 M4 行）· 003（代码组织：`platform/storage`/`queue` 位置与端口归属）
